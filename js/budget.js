(function() {
  function applyCheckbookActuals() {
    ...
    let actuals = {};
    try {
      actuals = JSON.parse(localStorage.getItem("checkbookActuals")) || {};
    } catch (error) {
      actuals = {};
    }
  ...
  function saveCheckbookActuals() {
    ...
    try {
      localStorage.setItem("checkbookActuals", JSON.stringify(actuals));
    } catch (error) {
      console.warn("Unable to save checkbook actuals.", error);
    }
  }
