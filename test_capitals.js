...
async function testCapitals() {
    const { error } = await supabase.from('paylog submissions').select('Jul').limit(1);
    console.log('Select Jul error:', error ? error.message : 'SUCCESS');
}
