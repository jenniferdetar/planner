...
export function normalizePaylogMonth(raw: string | null | undefined): string | null {
  if (!raw) return null
...
export function useIcaapDashboard() {
  ...
  const [paylogSubmission, setPaylogSubmission] = useState<any[]>([])
  const [approvalDates, setApprovalDates] = useState<any[]>([])
  ...
  const rows = hoursWorked.map(hw => {
    const name = hw['Name']
    const ps = paylogSubmission.find((p: any) => p['Employee Name'] === name) || {}
    const ad = approvalDates.find((a: any) => a['Name'] === name) || {}
    return { name, hw, ps, ad }
  })

  // Import parsed CSV rows: [{ name, monthCol, dateValue }]
  async function importPaylogRows(parsedRows: { name: string; monthCol: string; dateValue: string }[]) {
  ...
  async function importHoursRows(parsedRows: { name: string; updates: Record<string, number> }[]) {
    const results = await Promise.all(
