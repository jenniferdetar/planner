...
export function useMasterTasks(userId: string | null) {
  ...
  async function addTask(title: string, priority: string, category = '') {
...
export function useDailyTasks(userId: string | null, selectedDate: Date) {
  const [tasks, setTasks] = useState<any[]>([])
...
export function useMeetings(userId: string | null, selectedDate: Date) {
  ...
  async function addMeeting(title: string, hour: number, color: string, startTime: string, endTime: string) {
  ...
  async function bulkAddMeetings(events: any[]) {
    ...
    const { data: existing } = await supabase
      ...
      .eq('user_id', userId)
      .in('date', dates)
    const existingKeys = new Set((existing || []).map((r: any) => `${r.date}|${r.title}|${r.start_time}`))
    ...
    if (toInsert.length) {
      ...
      if (toInsert.some((e: any) => e.date === dateStr)) {
        const { data } = await supabase
...
export function useNotes(userId: string | null, selectedDate: Date) {
...
export function useTaskCounts(userId: string | null) {
...
export function useMeetingsInRange(userId: string | null, startDate: Date, endDate: Date) {
  const [meetings, setMeetings] = useState<any[]>([])
