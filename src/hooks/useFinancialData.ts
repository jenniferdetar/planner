...
export function useTransactions(userId: string | null) {
  ...
  async function addTransaction(fields: Record<string, any>) {
...
export function useBills(userId: string | null) {
  ...
  async function addBill(fields: Record<string, any>) {
    const { data } = await supabase
      .from('bills')
      .insert({ ...fields, user_id: userId })
      .select().single()
...
export function useFinancialGoals(userId: string | null) {
  ...
  async function addGoal(fields: Record<string, any>) {
    const { data } = await supabase
      .from('financial_goals')
      .insert({ ...fields, user_id: userId })
      .select().single()
...
export function usePaychecks(userId: string | null) {
  ...
  async function addPaycheck(pay_date: string, amount: number) {
    const { data } = await supabase
      .from('paychecks')
  ...
  async function updatePaycheckAmount(id: string, amount: number) {
  ...
  async function togglePaycheckBill(id: string, billId: string) {
    const pc = paychecks.find(p => p.id === id)
    const ids: string[] = pc.paid_bill_ids || []
