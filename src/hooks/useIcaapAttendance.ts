...
export function useIcaapAttendance(userId: string | null) {
  ...
  async function updateNotes(meeting_date: string, member_name: string, notes: string): Promise<void> {
    const existing = records.find(
      r => r.meeting_date === meeting_date && r.member_name === member_name
    )
    if (existing) {
