...
export function useWorkLocations() {
  ...
  useEffect(() => {
    supabase
      ...
      .order('"School Name"')
      .limit(2000)
      .then(({ data }) => setLocations((data || []).map((r: any) => r['School Name'])))
  }, [])
  return locations
}

export function useCseaIssues(userId: string | null) {
  ...
  async function addIssue(fields: Record<string, any>) {
    const { data } = await supabase
      .from('csea_issues')
...
export function useMemberInteractions(userId: string | null) {
  ...
  async function addInteraction(fields: Record<string, any>) {
    const { data } = await supabase
      .from('member_interactions')
  ...
  async function updateInteraction(id: string, fields: Record<string, any>) {
    const { data } = await supabase
      .from('member_interactions')
      .update(fields)
      .eq('id', id)
