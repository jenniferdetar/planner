...
export async function fetchCalendarEvents(providerToken: string, startDate: Date, endDate: Date): Promise<CalendarTimeBlock[]> {
  ...
  const results = await Promise.allSettled(
    CALENDARS.map(async (cal) => {
      ...
      return (data.items || []).map((evt: any): Omit<CalendarTimeBlock, 'hour' | 'durationMins' | 'startLabel' | 'endLabel'> => ({
        id: `gcal_${evt.id}`,
  ...
  const authFailed = results.some(r => r.status === 'rejected' && (r as PromiseRejectedResult).reason?.message === 'GOOGLE_AUTH_EXPIRED')
...
export function eventToTimeBlock(event: Omit<CalendarTimeBlock, 'hour' | 'durationMins' | 'startLabel' | 'endLabel'>): CalendarTimeBlock {
