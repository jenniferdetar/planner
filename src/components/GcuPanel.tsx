...
const STATUS_COLORS: Record<string, string> = { 'not started': '#bbb', 'in progress': '#f0a040', 'completed': '#5cb85c' }

interface GcuPanelProps {
  onPushToAsana: () => Promise<void>
  pushing: boolean
}
...
export default function GcuPanel({ onPushToAsana, pushing }: GcuPanelProps) {
...
function CourseGroup({ label, courses, statuses, expanded, onToggle, onCycle }: CourseGroupProps) {
  return (
