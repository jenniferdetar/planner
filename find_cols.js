...
async function findColumns() {
    const possibleNames = ['name', 'Name', 'employee', 'Employee', 'full_name', 'Full Name', 'id', 'ID'];
    for (const n of possibleNames) {
        const { error } = await supabase.from('paylog submissions').select(n).limit(1);
        if (!error) {
            console.log(`FOUND COLUMN: "${n}"`);
        } else {
            console.log(`Column "${n}" not found: ${error.message}`);
        }
    }
}
