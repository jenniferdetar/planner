const goalsPage = (() => {
  ...
  async function autoImportGoalsIfNeeded() {
    try {
      ...
      const storedVersion = localStorage.getItem(AUTO_IMPORT_VERSION_KEY);
      const importHash = hashString(jsonString);
      const storedHash = localStorage.getItem(AUTO_IMPORT_HASH_KEY);
      ...
      localStorage.setItem(AUTO_IMPORT_FLAG, 'true');
      localStorage.setItem(AUTO_IMPORT_VERSION_KEY, importVersion);
      localStorage.setItem(AUTO_IMPORT_HASH_KEY, importHash);
...
document.addEventListener('DOMContentLoaded', goalsPage.initializePage);
