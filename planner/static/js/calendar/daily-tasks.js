const dailyTasks = (() => {
  ...
  function loadTasks() {
    return opusStorage.getMetadata('dailyTasks') || {};
  }

  function saveTasks(tasksByDate) {
    opusStorage.updateMetadata('dailyTasks', tasksByDate);
  }

  function loadHabits() {
    return opusStorage.getHabits();
  }
  ...
  function loadHabitStatus() {
    const allStatus = {};
    // This is inefficient but keeps legacy compatibility
    // Better to refactor daily-tasks.js to only care about today
    const dateInput = document.getElementById('task-date');
    if (dateInput) {
      const dateKey = getDateKey(dateInput);
      allStatus[dateKey] = opusStorage.getHabitStatus(dateKey);
    }
    return allStatus;
  }
  ...
  function saveHabitStatus(statusByDate) {
    // Note: statusByDate in this legacy script is the whole status object
    // but opusStorage expects individual updates or a different structure.
    // For simplicity, let's update it per date.
    const dateInput = document.getElementById('task-date');
    const dateKey = getDateKey(dateInput);
    const statusForDate = statusByDate[dateKey] || {};
    Object.entries(statusForDate).forEach(([habitId, completed]) => {
      opusStorage.updateHabitStatus(dateKey, habitId, completed);
    });
  }
