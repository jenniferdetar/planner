window.calendarEmbed = (() => {
  const renderCalendar = () => {
    return new Promise((resolve) => {
      ...
      if (!['calendar.html', '', 'calendar'].includes(currentPage) && cleanPath !== '/calendar' && window.location.pathname !== '/') {
        return resolve();
      }
      ...
      if (document.querySelector('script[data-calendar-embed-script]')) {
        if (window.calendarPage && window.calendarPage.initialize) {
          window.calendarPage.initialize().then(resolve);
        } else {
          resolve();
        }
        return;
      ...
      script.onerror = () => {
        console.error('Failed to load calendar.js');
        resolve();
      };
  ...
  return { initialize: renderCalendar };
})();
