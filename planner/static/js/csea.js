window.cseaPage = (() => {
  ...
  function applyMemberIds(nameToId) {
    if (!nameToId || !Object.keys(nameToId).length) return
    ...
    if (updated) {
      if (window.opusStorage) {
        opusStorage.setCseaIssues(issues)
      }
      renderIssues()
    }
  }

  async function loadMemberIdsFromSupabase() {
    if (!window.supabaseClient || typeof requireSupabaseUser !== 'function') return
    ...
    try {
      ...
      const { data, error } = await window.supabaseClient
        .from('csea_members')
        .select('member_id, full_name, first_name, last_name')
      ...
      data.forEach(member => {
        ...
        if (!memberId) return
        let fullName = (member.full_name || '').trim()
        if (!fullName) {
        ...
        if (!fullName) return
        nameToId[fullName.toLowerCase()] = memberId
      })

      applyMemberIds(nameToId)
    } catch (err) {
      console.warn('Could not load member IDs from Supabase', err)
    }
  }
  ...
  function attachDropdownListeners() {
    document.querySelectorAll('.type-select').forEach(select => {
      select.addEventListener('change', (e) => {
        ...
        if (idx !== -1) {
          issues[idx].type = e.target.value
          opusStorage.setCseaIssues(issues)
        }
        updateStats()
      })
    ...
    document.querySelectorAll('.steward-select').forEach(select => {
      select.addEventListener('change', (e) => {
        ...
        const idx = issues.findIndex(i => i.id === issueId)
        if (idx !== -1) {
          issues[idx].steward = e.target.value
          opusStorage.setCseaIssues(issues)
        }
        updateStats()
      })
    ...
    document.querySelectorAll('.priority-select').forEach(select => {
      select.addEventListener('change', (e) => {
        ...
        const idx = issues.findIndex(i => i.id === issueId)
        if (idx !== -1) {
          issues[idx].priority = e.target.value
          opusStorage.setCseaIssues(issues)
        }
        updateStats()
      })
    ...
    document.querySelectorAll('.status-select').forEach(select => {
      select.addEventListener('change', (e) => {
        ...
        const idx = issues.findIndex(i => i.id === issueId)
        if (idx !== -1) {
          issues[idx].status = e.target.value
          opusStorage.setCseaIssues(issues)
        }
        updateStats()
      })
  ...
  async function initialize() {
    try {
      ...
      const savedIssues = opusStorage.getCseaIssues()
      ...
      renderIssues()
      await loadMemberIdsFromSupabase()
      await loadMemberIdsFromCsv()
      ...
      if (window.cseaMeetingNotes && typeof window.cseaMeetingNotes.initialize === 'function') {
        await window.cseaMeetingNotes.initialize();
      }
      if (window.cseaNotesModule && typeof window.cseaNotesModule.initialize === 'function') {
        await window.cseaNotesModule.initialize();
      }
      if (window.cseaUI && typeof window.cseaUI.initialize === 'function') {
        window.cseaUI.initialize();
      }

    } catch (error) {
      console.error('Failed to initialize CSEA:', error);
    }
  }
