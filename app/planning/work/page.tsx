...
export default function WorkPlannerPage() {
  ...
  return (
    <div className="p-3 md:p-6 w-full bg-[#fdfdfd] min-h-screen">

      {/* Priorities Section */}
      <section className="mb-8">
        <div className="flex items-center gap-3 mb-6">
          <Target className="text-[#9ADBDE]" size={20} />
          <h2 className="text-xl font-black text-[#0a2f5f] tracking-tight">Strategic Priorities</h2>
          <div className="h-px flex-grow bg-gradient-to-r from-[#0a2f5f]/20 to-transparent"></div>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            { key: 'p1', label: 'Primary Directive', icon: <Award className="text-amber-500" size={16} />, placeholder: 'Priority #1' },
            { key: 'p2', label: 'Secondary Goal', icon: <Target className="text-emerald-500" size={16} />, placeholder: 'Priority #2' },
            { key: 'p3', label: 'Tactical Objective', icon: <Zap className="text-indigo-500" size={16} />, placeholder: 'Priority #3' },
            { key: 'look-forward', label: 'Future Milestone', icon: <Clock className="text-rose-500" size={16} />, placeholder: 'One thing I&apos;m looking forward to' },
            ...
            <div key={item.key} className="bg-white p-5 rounded-3xl border-2 border-gray-50 shadow-lg shadow-gray-200/40 group hover:border-[#9ADBDE]/30 transition-all">
              ...
              <div
                ...
                className="w-full min-h-[60px] p-4 bg-slate-50/50 border-2 border-transparent focus:border-[#9ADBDE]/20 rounded-xl outline-none font-bold text-[#0a2f5f] transition-all text-sm leading-relaxed relative"
              >
                {priorities[item.key] || ''}
                {!priorities[item.key] && (
                  <span className="absolute inset-0 p-4 pointer-events-none text-gray-300 italic font-medium">
                    {item.placeholder}
                  </span>
                )}
              </div>
            ...
            {DAYS.map((day: string, dIdx: number) => {
              ...
              return (
                <div key={day} className={`flex-1 min-w-[200px] border-r border-gray-50 last:border-r-0 ${
                  ...
                  <div className={`p-3 text-center border-b font-black ${
      ...
      <section className="bg-white rounded-3xl border border-gray-100 shadow-lg p-6 mb-8">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-black text-[#0a2f5f] flex items-center gap-2">
            ...
            goals.map((goal, gIdx) => (
              <button
                ...
                className={`flex items-center gap-3 p-3 rounded-xl border text-left transition-all group ${
                  ...
                  : 'bg-white border-gray-50 hover:border-orange-100 hover:bg-slate-50'
                }`}
              >
