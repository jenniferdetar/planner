...
export default function BudgetOverviewPage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto bg-[#fdfdfd] min-h-screen font-handwriting">
      ...
      <section className="bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden mb-12 relative">
        ...
        <div className="p-10 border-b border-gray-50 bg-[#f8fafc] flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-black text-[#0a2f5f] tracking-tight">Ledger Allocation Breakdown</h2>
          </div>
          {saving && (
            <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-full border border-blue-100 animate-pulse">
              <Save size={14} />
              <span className="text-sm font-black tracking-widest">Encrypting...</span>
            </div>
          )}
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white">
                <th className="p-8 font-black text-[#0a2f5f] text-sm tracking-[0.2em] border-b-2 border-gray-50">Operational Item</th>
                <th className="p-8 font-black text-[#0a2f5f] text-sm tracking-[0.2em] border-b-2 border-gray-50">Class</th>
                <th className="p-8 font-black text-[#0a2f5f] text-sm tracking-[0.2em] border-b-2 border-gray-50 text-right">Budgeted</th>
                <th className="p-8 font-black text-[#0a2f5f] text-sm tracking-[0.2em] border-b-2 border-gray-50 text-right">Actual</th>
                <th className="p-8 font-black text-[#0a2f5f] text-sm tracking-[0.2em] border-b-2 border-gray-50 text-right">Variance</th>
              </tr>
            </thead>
                ...
                return (
                  <tr key={item.item} className="group hover:bg-slate-50 transition-colors">
                    <td className="p-8">
                      ...
                      <div className="text-sm font-black text-[#FFC68D] tracking-widest">{item.category}</div>
                    </td>
                    <td className="p-8">
                      <span className={`text-sm font-black tracking-widest px-3 py-1 rounded-full border ${item.type === 'Fixed' ? 'bg-gray-50 text-gray-400 border-gray-100' : 'bg-blue-50 text-blue-500 border-blue-100'}`}>
                        {item.type}
                      </span>
                    </td>
            ...
            <tfoot className="bg-slate-50 border-t-4 border-white">
              <tr className="font-black">
                <td colSpan={2} className="p-10 text-[#0a2f5f] tracking-widest text-sm">Consolidated Registry Totals</td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-[#FFC68D]/10 p-10 rounded-3xl border-2 border-[#FFC68D]/20">
          <h4 className="text-[#0a2f5f] font-black tracking-[0.2em] text-sm mb-6">Auditor&apos;s Certification</h4>
        ...
        <div className="bg-slate-50 p-10 rounded-3xl border-2 border-slate-100 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-sm font-black text-gray-400 tracking-widest mb-1">Audit Status</div>
                <div className="text-xl font-black text-[#0a2f5f]">Records Verified</div>
              </div>
            </div>
            <ArrowUpRight className="text-[#0a2f5f] opacity-20" size={32} />
          </div>
          <div className="mt-8 text-sm font-black tracking-[0.4em] text-gray-400">Ledger Compliance Registry v4.0</div>
        </div>
      </div>

      <footer className="mt-20 py-12 border-t border-gray-100 text-center">
        <p className="text-gray-400 text-sm font-black tracking-[0.4em]">Official Fiscal Audit Registry © 2026</p>
      </footer>
    </div>
  );
}
...
function SummaryCard({ title, value, icon, sub, isVariance }: { title: string; value: number; icon: React.ReactNode; sub: string; isVariance?: boolean }) {
  ...
  return (
    <div className="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-2xl relative overflow-hidden group hover:-translate-y-2 transition-all duration-500">
      <div className="relative z-10">
        <div className="flex justify-between items-start mb-8">
          ...
          {isVariance && (
            <span className={`text-sm font-black tracking-widest px-3 py-1 rounded-full border ${isNegative ? 'bg-rose-50 text-rose-500 border-rose-100' : 'bg-emerald-50 text-emerald-500 border-emerald-100'}`}>
              {isNegative ? 'Over Budget' : 'Within Budget'}
            </span>
          )}
        </div>
        <p className="text-sm font-black text-gray-400 tracking-[0.2em] mb-2">{title}</p>
        <p className={`text-3xl font-black tracking-tighter ${isVariance ? (isNegative ? 'text-rose-500' : 'text-emerald-600') : 'text-[#0a2f5f]'}`}>
          {new Intl.NumberFormat('en-Us', { style: 'currency', currency: 'Usd', minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(value)}
        </p>
        <p className="mt-4 text-sm font-bold text-gray-300 tracking-widest">{sub}</p>
      </div>
