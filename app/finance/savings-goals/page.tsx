...
export default function SavingsGoalsPage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto bg-[#fdfdfd] min-h-screen font-handwriting">
      ...
      <section className="relative bg-white p-8 md:p-16 rounded-3xl shadow-2xl border border-gray-100 overflow-hidden mb-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 relative z-10">
          <div>
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-xl font-black text-[#0a2f5f] tracking-tight">Financial Vision 2026</h2>
            </div>
            <p className="text-gray-500 font-medium leading-relaxed italic mb-8 text-sm">
            ...
            <div className="relative mb-8">
              <input 
                ...
                className="w-full p-6 bg-[#f8fafc] border-2 border-gray-50 rounded-2xl text-lg outline-none focus:bg-white focus:border-[#9ADBDE] transition-all shadow-inner"
              />
              <button 
                onClick={addGoal}
            ...
            <div className="space-y-4">
              {goals.map((goal, idx) => (
                <div key={idx} className="group flex items-center justify-between p-6 bg-white border-2 border-gray-50 rounded-2xl hover:border-[#9ADBDE]/30 transition-all shadow-sm">
                  <div className="flex items-center gap-4">
                    <div className="w-8 h-8 rounded-lg bg-[#9ADBDE]/10 flex items-center justify-center text-[#9ADBDE]">
                      <Target size={16} />
                    </div>
                    <span className="text-lg text-[#0a2f5f] italic">{goal}</span>
                  </div>
                  <button 
              ...
              {goals.length === 0 && (
                <div className="text-center py-12 border-4 border-dashed border-gray-50 rounded-3xl">
                  <p className="text-gray-300 font-black tracking-widest text-sm">No Registered Objectives</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-8">
            <div className="p-8 bg-slate-50 rounded-3xl border-2 border-slate-100 flex flex-col justify-between h-full">
              <div>
                <div className="flex items-center gap-3 mb-6">
                  <ShieldCheck className="text-[#0a2f5f]" size={24} />
                  <h3 className="text-lg font-black text-[#0a2f5f] tracking-tight">Security Protocol</h3>
                </div>
                <p className="text-gray-500 font-medium leading-relaxed italic mb-8 text-sm">
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-black text-gray-400 tracking-widest">Reserve Status</span>
                  <span className="text-sm font-black text-[#9ADBDE] tracking-widest">Active Monitoring</span>
                </div>
      ...
      <section className="mb-16">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2 className="text-xl font-black text-[#0a2f5f] tracking-tight text-center lg:text-left">Strategic Reserve Matrix</h2>
          <div className="h-px flex-grow bg-gradient-to-r from-[#0a2f5f]/20 to-transparent"></div>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {SAVINGS_CATEGORIES.map(cat => (
            <div key={cat.name} className="bg-white rounded-3xl border border-gray-100 shadow-xl overflow-hidden group hover:-translate-y-2 transition-all duration-500">
              <div className="p-8 bg-slate-50 border-b border-gray-100 flex justify-between items-center">
                <h3 className="text-sm font-black text-[#0a2f5f] tracking-tight">{cat.name}</h3>
              ...
              <div className="p-8 space-y-6">
                  ...
                  {cat.items.map(item => (
                    <div key={item.item} className="flex justify-between items-center border-b border-gray-50 pb-4 last:border-0 last:pb-0">
                      ...
                      <div className="text-right">
                        <div className="text-sm font-black text-[#0a2f5f]">{formatCurrency(item.monthly * 3)} <span className="text-gray-300 ml-1 font-bold">3M</span></div>
                        <div className="text-xs font-bold text-gray-300">{formatCurrency(item.monthly * 6)} 6M</div>
                      </div>
                    </div>
                  ))}
                </div>
                <div className="pt-6 border-t-2 border-dashed border-gray-100 flex justify-between items-end">
                  <div>
                    <div className="text-sm font-black text-gray-400 tracking-widest mb-1">Total Target</div>
                    <div className="text-xl font-black text-[#0a2f5f]">{formatCurrency(getSubtotal(cat.items) * 6)}</div>
                  </div>
                  <div className="text-sm font-black text-[#9ADBDE] tracking-widest bg-[#9ADBDE]/5 px-3 py-1 rounded-full border border-[#9ADBDE]/10">
                    6 Months
                  </div>
                </div>
              </div>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#0a2f5f] via-[#2d5a8e] to-[#6a93cb] rounded-3xl p-12 md:p-20 text-white shadow-2xl">
        <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-12">
          <div className="max-w-xl text-center lg:text-left">
            <h2 className="text-3xl font-black mb-6 tracking-tight ">Consolidated Reserve Targets</h2>
          ...
          <div className="flex flex-col md:flex-row gap-8 w-full lg:w-auto">
            <div className="bg-white/10 backdrop-blur-xl p-10 rounded-3xl border border-white/20 flex-1 lg:w-72 text-center group hover:bg-white/20 transition-all">
              <TrendingUp className="text-[#9ADBDE] mx-auto mb-6" size={32} />
              <p className="text-sm font-black tracking-[0.3em] opacity-60 mb-2 text-white">3 Month Liquidity</p>
              <p className="text-3xl font-black text-white tracking-tighter">{formatCurrency(grandTotalMonthly * 3)}</p>
            </div>
            <div className="bg-white/10 backdrop-blur-xl p-10 rounded-3xl border border-white/20 flex-1 lg:w-72 text-center group hover:bg-white/20 transition-all">
              <Landmark className="text-[#FFC68D] mx-auto mb-6" size={32} />
              <p className="text-sm font-black tracking-[0.3em] opacity-60 mb-2 text-white">6 Month Fortress</p>
              <p className="text-3xl font-black text-white tracking-tighter">{formatCurrency(grandTotalMonthly * 6)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="absolute -bottom-24 -right-24 text-[15rem] opacity-5 pointer-events-none font-black text-white">Safe</div>
      </section>

      <footer className="mt-20 py-12 border-t border-gray-100 text-center">
        <p className="text-gray-400 text-sm font-black tracking-[0.4em]">Official Capital Reserves Registry © 2026</p>
      </footer>
    </div>
  );
}
